...
function App() {
  ...
  return (
    <>
      <Board position={position} setPosition={setPosition} moveCounter={moveCounter} setMoveCounter={setMoveCounter}
             savedPosition={savedPosition} setSavedPosition={setSavedPosition} records={records} setRecords={setRecords} teleportDirection={teleportDirection} 
             setTeleportDirection={setTeleportDirection} rows={rows} setRows={setRows} columns={columns} setColumns={setColumns} settings={settings} setSettings={setSettings}
      />
    </>
  );
}
